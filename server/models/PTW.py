...
class PTW:
    ...
    class Approval:
        ...
        __updateApprovalStatus() to derive approval_status. role/department
        snapshot the actor's role and department at approval time (stamped
        server-side), so replay stays valid even if the user is later deleted
        or re-roled; records predating the snapshot fall back to the live
        user record (see roleDept())."""
        ...
        def __init__(self, action = None, username: str = None, timestamp: str = None, comment: str = None,
            ...
            timestamp, optional comment, and the actor's role/department
            snapshot."""
            self.action = action
        ...
        def __str__(self):
            ...
            when (with department for a USER approver), falling back to a
            deleted-user placeholder if the acting username no longer
            resolves in globalData.allUsers."""
            user = globalData.allUsers.get(self.username)
            if user is None:
    ...
    class GasTest:
        ...
        def __init__(self, username: str = None, timestamp: str = None, shift: str = None,
            ...
            """Initialize a gas test record with who recorded it, when, which shift it's
            credited toward, its per-gas readings, and an optional comment."""
            self.username = username
            self.timestamp = timestamp
            self.shift = shift
    ...
    def requiredApprovers(self) -> list[list['PTW.Approver']]:
        ...
        and HSE Engineer (HSE) in parallel; and, for Hot Work/Confined Space
    ...
    def needsGasTestNow(self) -> bool:
        """True whenever this PTW requires an initial gas test and doesn't yet have an
        acceptable one for the shift a reading taken right now would count toward,
        regardless of whether its PA has actually requested to run this shift. Scoped to
        APPROVED and not yet CLOSED: an under-review permit can't run yet regardless, and
        a closed one never will again, so neither should keep demanding a fresh reading
        every shift."""
        return (
            self.approval_status == PTW.ApprovalStatus.APPROVED
