...
- **Exceeded 14-shift validity** (`needsCloseAlarm()`) — each row (`PTW #{id} — {description}`) has **View** and **Close** buttons, plus a **Close All** bulk button above the section that requests closing every not-yet-actioned row in it after one confirmation (rather than one confirmation per PTW).
- **Run cycle shift ended** (`isRunCycleShiftExpired()`) — each row has **View**, **Hold**, and **Close** buttons.

Each section's header is a checkable `QToolButton` (`_collapsibleSection`, chevron-down/chevron-right icon) that shows/hides that section's rows — both start expanded, since surfacing everything is the whole point of the dialog; collapsing one is purely a decluttering aid while working through a long list, it never affects which PTWs are actually alarmed.
