...
**`getStatus()`** layers the isolate cycle on top of the approval chain: `deisolate_isolator` set → `Closed`; `sanction_isolator` set and not yet reversed by `reisolate_isolator` → `Sanctioned` (both `sanction_*`/`reisolate_*` cycles still deferred, so unreachable today); `isolate_isolator` or `reisolate_isolator` set → `Active`; else, if `isolate_requestor` is set: `isolate_issuing_action == Approved` → `Pending` (awaiting isolator), `isolate_issuing_action == Returned` → falls through to `getApprovalStatus()` (i.e. `Approved`, ready to re-request), otherwise → `Isolate Confirming` (awaiting IA); with no `isolate_requestor` at all, falls through to `getApprovalStatus()` (`Requested`/`Returned`/`Approved`). Row/type coloring (`backgroundColor()`/`foregroundColor()`/`backgroundColorForType()`) mirrors `PTWData`'s pattern: Mechanical=gray, Electrical=yellow, Self=green, Protective System=red, Other=neutral gray.

**Tab routing is per-viewer, like PTW's Requested/Under Review split** (`MainWindow.refreshCertificatesGUI()`), one tab per `Status` value: **Requested** → **Under Review** (per-viewer, whoever's approval stage is current) → **Approved** → **Isolate Confirming** → **Pending** → **Active** → **Sanctioned** → **Closed**. A viewer whose approval stage already passed (e.g. Issuing, once a `Protective`-type certificate has moved on to PDH) falls back to **Requested** as a tracking view — `IssuingMainWindow` doesn't have a Requested button today, so that specific tracking case is a known, accepted gap (rare: only affects multi-stage `Protective` certificates after Issuing's own stage is done).

Per-row menu visibility uses a new `TablePTWs.MenuOption(..., visibleFor=lambda cert: ...)` predicate (default `None` = always visible), checked in `TableIsolationCertificates.showContextMenu` only — `TablePTWs`'s own context menu is untouched, so this doesn't affect PTW menus. Used defensively on the three isolate-cycle actions (e.g. `optionConfirmIsolateIC.visibleFor` checks `getStatus() == Isolate Confirming`) even though tab routing alone already guarantees the right rows end up in the right tab — belt-and-suspenders against a stale row lingering between an action and the next refresh.

**Roles wired:**
- `UserMainWindow` — Requested / Approved (+ Request Isolate) / Isolate Confirming (view-only, tracks their own request) / Pending / Active / Sanctioned / Closed tabs; FAB on the Requested tab creates a new certificate (`TableIsolationCertificates.addNewCertificateDialog()`), submitting via `POST /isolation-certificates`.
- `IssuingMainWindow` — Under Review (with Accept/Request Edits) / Approved (view-only) / Isolate Confirming (+ Confirm Isolate/Return Isolate Request) / Pending (view-only) / Active / Sanctioned / Closed tabs.
...
- `IsolatorMainWindow` *(new — `UserRoles.ISOLATOR` previously had no dedicated window at all)* — Pending (+ Complete Isolation) / Active / Sanctioned tabs only, no PTW tabs, FAB permanently hidden. No Approved or Isolate Confirming tab — nothing for the isolator to do at either stage.
...
- **Isolation Items** — the embedded `TableIsolationItems` list. Lock #/Lock Box # are always read-only here regardless of mode — they're filled in by the isolator at confirmation time (still deferred — `isolate-execute` today only stamps `isolate_isolator`/timestamp, it doesn't yet open an editable items view), never by the requestor.
