...
- **Recording, one reading credited to a whole selection at once**: `POST /ptws/gas-test`, `GAS_TESTER` role only — body is `{ptw-ids: [...], readings: [{gas, percentage}, ...], timestamp, comment}`. The **Record Gas Test** menu option is an `allAtOnce=True` `TablePTWs.MenuOption` (same mechanism as `Archive`/`Export`), so a Gas Tester multi-selects several PTWs tested together in one pass and enters the readings only once — `MainWindow.recordGasTestPTW(rows, ptws)` opens a single `DialogGasTest` for the whole selection and submits it as one request. The server resolves and stamps `shift` itself from `timestamp` (never trusted from the client, and the same for every PTW in the batch); `PtwsDb.addGasTestPTWs` appends the identical `PTW.GasTest` to every given PTW's `gas_tests JSONB[]` column in a single `UPDATE ... WHERE id IN (...)` (one independent entry per test per PTW, same append-only pattern as `approvals` — nothing is ever patched in place, unlike `run_cycles`). UI: `DialogGasTest` collects one required percentage per `GAS_TEST_TYPES` entry plus an optional comment, labeled with every selected PTW's id; `DialogPTW`'s History tab gains a third Timeline pane (`_buildGasTestTimelinePane`) once a PTW requires or already has a gas test recorded.
...
| POST   | `/ptws/gas-test`            | Gas Tester records the same initial gas test reading for one or more PTWs at once (400s if any given PTW doesn't require one — see [Initial Gas Test](#initial-gas-test)) |
...
| `dialogs/DialogGasTest.py` | *(added 2026-09-07)* Gas Tester's "Record Gas Test" dialog, opened once for a whole multi-selection of PTWs (`allAtOnce` menu option) — one required percentage reading per `PTW.GAS_TEST_TYPES` entry plus an optional comment; `getReadings()`/`getComment()` return the collected fields for `ClientRequests.recordGasTestPTW`, submitted for every selected PTW at once. See [Initial Gas Test](#initial-gas-test) |
