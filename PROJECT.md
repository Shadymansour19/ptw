...
**`DialogIC` is tabbed, mirroring `DialogPTW`'s pattern** (both import the shared `TabButton`/`lightenColor`/`Timeline` helpers from `widgets/UiUtils.py` rather than duplicating them): a colored tab bar above a `QStackedWidget`, background/accent/highlight color driven by the IC's type (same palette as the row coloring above).
