...
**`getStatus()`** layers the isolate-execution cycle (still dormant) on top of the approval chain: `deisolate_isolator` set → `Closed`; `sanction_isolator` set and not yet reversed by `reisolate_isolator` → `Sanctioned`; `isolate_isolator` or `reisolate_isolator` set → `Active`; `isolate_requestor` set → `Pending`; otherwise falls through to `getApprovalStatus()` (`Requested`/`Returned`/`Approved`). Row/type coloring (`backgroundColor()`/`foregroundColor()`/`backgroundColorForType()`) mirrors `PTWData`'s pattern: Mechanical=gray, Electrical=yellow, Self=green, Protective System=red, Other=neutral gray.

**Tab routing is per-viewer, like PTW's Requested/Under Review split** (`MainWindow.refreshCertificatesGUI()`): a fully-approved certificate with no isolate request yet routes to **Approved** (its own tab, distinct from Pending); once `isolate_requestor` is set (manually or via `isolate_asap` auto-stamp) it moves to **Pending** (awaiting IA confirmation, then isolator execution — both still deferred, so it just sits there for now); otherwise a certificate whose current pending approval stage matches the viewer's role+department shows in their **Under Review** tab, everyone else sees **Requested**. A viewer whose stage already passed (e.g. Issuing, once a `Protective`-type certificate has moved on to PDH) falls back to **Requested** as a tracking view — `IssuingMainWindow` doesn't have a Requested button today, so that specific tracking case is a known, accepted gap (rare: only affects multi-stage `Protective` certificates after Issuing's own stage is done).

**Approved → Pending handoff — implemented:** the **Approved** tab has a **Request Isolate** action (`optionRequestIsolateIC`, `UserMainWindow` only — Issuing sees Approved read-only) → `MainWindow.requestIsolateCertificate` → `ClientRequests.requestIsolateCertificate` → `POST /isolation-certificates/isolate-request`, which 403s unless the certificate's `getStatus()` is currently `Approved`, then stamps `isolate_requestor`/`isolate_requestor_timestamp` and broadcasts an unrestricted `isolation_certificate_isolate_request` SSE event. This is the "goes to IA to confirm, then goes to isolator" handoff described by the flow — the confirm-by-IA and execute-by-isolator steps themselves remain the deferred isolate-execution cycle below; this only wires the first step (getting a certificate from Approved into Pending).

**Roles wired:**
- `UserMainWindow` — Requested / Approved (+ Request Isolate) / Pending / Active / Sanctioned / Closed tabs; FAB on the Requested tab creates a new certificate (`TableIsolationCertificates.addNewCertificateDialog()`), submitting via `POST /isolation-certificates`.
- `IssuingMainWindow` — Under Review (with Accept/Request Edits) / Approved (view-only) / Pending / Active / Sanctioned / Closed tabs.
...
- `IsolatorMainWindow` *(new — `UserRoles.ISOLATOR` previously had no dedicated window at all)* — Pending / Active / Sanctioned tabs only, no PTW tabs, FAB permanently hidden. No Approved tab — there's nothing for the isolator to do until IA confirmation (deferred) happens.
...
- **Isolation Items** — the embedded `TableIsolationItems` list. Lock #/Lock Box # are always read-only here regardless of mode — they're filled in by the isolator at confirmation time (still deferred), never by the requestor.
...
- **PTW Linkage** — also readonly-only, exposes the fields deferred for the PTW-linkage phase — `primary_ptw`, `latest_ptw`, `linked_ptws`, `held_by`, `is_physically_isolated` — all always empty until that phase lands. Fields are read-only `QLineEdit` (not `QLabel`) so the text is selectable/copyable.
...
**Not implemented yet — explicitly deferred:** the isolate-execution cycle itself (a user requests to isolate — automatically if `isolate_asap`, else manually — Issuing confirms, the isolator carries it out and confirms), plus its hold/sanction-for-test and de-isolate variants, and linking a certificate to a PTW. The approval chain above is fully wired; only what happens *after* `Approved` remains dormant scaffolding (fields exist, `getStatus()` already accounts for them, no endpoints set them yet).

Server-side: `GET /isolation-certificates` (list, department-scoped for `UserRoles.USER` only), `POST /isolation-certificates` (create), `POST /isolation-certificates/approvals` (staged approve/reject).
