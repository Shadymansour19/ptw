...
| `DialogIC.py`               | (renamed from `DialogIsolationCertificate.py` 2026-07-25) IC create/view dialog, tabbed like `WidgetPTW.DialogPTW` (Basic Info / Isolation Items / P&ID / Wiring / PSIC / History / PTW Linkage — the last two only in readonly mode); `new`/`readOnly` flags mirror `WidgetPTW.DialogPTW`. The PSIC tab holds `is_psic`, `psic_reasons` (multi-select), `psic_moc_number`, and the three PSIC description fields, plus the "Autofill from Tag" button (see [Isolation Management](#isolation-management)). PTW Linkage rows have **View** and (non-Guest) **Unlink** buttons. |
