...
**Linking a certificate to a PTW — implemented (link only, not hold/unlink):** a **Link to PTW** action (`optionLinkPTWToIC`, `UserMainWindow` and `IssuingMainWindow` only) pops a plain `QInputDialog.getText` asking for a PTW #, then → `MainWindow.linkPTWToCertificate` → `ClientRequests.linkPTWToCertificate` → `POST /isolation-certificates/link-ptw`. Two gates, both expressed on the model so they're checked identically whether called from a menu predicate or the endpoint:
- `IsolationCertificate.isWindingDown()` — `True` once `Sanctioned`/`Deisolate Confirming`/`Closing`/`Closed`, i.e. the certificate is past the point where a new PTW should be attached. Drives `optionLinkPTWToIC.visibleFor` directly (menu predicates only ever receive the certificate, never a specific PTW, so this is the cert-only half of the check).
- `IsolationCertificate.canLinkPTW(ptw)` — `isWindingDown()` **and** the target PTW itself must be `approval_status == Approved` and `running_status == Not Running`: the window between a PTW being approved and it actually starting work. Not linkable while the PTW is still under review/returned, nor once run/hold/close has been requested or done. Used server-side, after `ptwDB.getPTWById(ptwId)`, and nowhere else — the client dialog only checks "not already linked to this cert" before submitting, deferring the PTW-state check to the (authoritative) server response.

`canLinkPTW` needs `PTWData`'s `ApprovalStatus`/`RunningStatus` enums, imported lazily inside the method body rather than at module scope — `PTWData.py` already imports the plain `Isolation` class at module scope, so a top-level `Isolation.py → PTWData.py` import would be a real circular import (same reasoning as `_viewLinkedPTW`/`_viewLinkedIC`'s lazy dialog imports above).

`IsolationCertificate.linkPTW(ptwId)` itself is the exact same logic as the flat `Isolation.linkPTW` it was copied from (now uncommented/implemented, having sat as a dead stub since the certificate's linkage fields were first scaffolded): un-holds the PTW if it was held, sets `primary_ptw` if this is the first link, appends to `linked_ptws`, updates `latest_ptw`, sets `is_physically_isolated = True`. `holdPTW`/`unlinkPTW`/`isReallyActive` remain commented-out/unimplemented for `IsolationCertificate` — those need the PTW side to call back into the certificate on its own RUNNING→HELD→CLOSED transitions, a bigger integration piece not covered by this "manual link" action.

**Not implemented yet — explicitly deferred:** the entire `sanction_*` (sanction-for-test) and `reisolate_*` cycles (no trigger, no confirm, no execute — the isolate/de-isolate cycles above are the template these will eventually mirror, per-cycle: `sanctionConfirming`/`reisolateConfirming` tabs alongside `isolateConfirming`/`deisolateConfirming`). Also still deferred: un-linking/holding a certificate's PTW as the PTW itself transitions (see above), clearing/editing `lock_num`/`lock_box_num` at de-isolate-execution time (the isolator physically removes the locks, but `deisolate-execute` doesn't touch `items` the way `isolate-execute` does), and a dedicated "Returned" tab + edit-and-resubmit flow for a returned certificate (PTW has `tabReturnedPTWs` + re-request; IC doesn't — a returned certificate just falls back into the Requested tab today).

Server-side: `GET /isolation-certificates` (list, department-scoped for `UserRoles.USER` only), `POST /isolation-certificates` (create), `POST /isolation-certificates/approvals` (staged approve/return), `POST /isolation-certificates/isolate-request` (Approved → Isolate Confirming), `POST /isolation-certificates/isolate-confirm` (Issuing approve/return, role-gated), `POST /isolation-certificates/isolate-execute` (Isolator complete, role-gated, optional per-item lock numbers), `POST /isolation-certificates/deisolate-request` (Active → Deisolate Confirming), `POST /isolation-certificates/deisolate-confirm` (Issuing approve/return, role-gated), `POST /isolation-certificates/deisolate-execute` (Isolator complete, role-gated) → `Closed`, `POST /isolation-certificates/link-ptw` (link a PTW, gated by `canLinkPTW`).
