...
**File:** `client/Login.py` — `PasswordLineEdit`, `_fillPasswordFor`, `_populateRememberedUsers`, `_onUsernameSelected`, `login`

The most-recent user's keyring password was auto-filled on load, any selected username's password was filled on selection, and the password field had an unmask (eye) toggle — so the plaintext was one click away.

**First pass (2026-08-19), scoped to the minimal mitigation:** `PasswordLineEdit` gained a `setToggleEnabled()` that disabled the eye button whenever a value was placed into the field programmatically, re-enabled the moment the user edited the field themselves (`textEdited`).

**Critical bypass found in that fix, same day:** `textEdited` fires on *any* edit, including appending a single throwaway character to the end of the still-present retrieved password — which re-enabled the toggle while the real password (plus that one extra character) was still sitting in the field, fully revealing it. Disabling the toggle was never sufficient on its own as long as the real password actually lived in the widget's text.

**Real fix, same day:** rewrote `PasswordLineEdit` so the real retrieved password is **never placed in the widget's text buffer at all**. `setRetrievedPassword()` stores it only in a private `_realValue` attribute and displays a fixed-length sentinel placeholder (`'•' * 10`, a constant length so even the dot-count can't leak the real password's length) in the field itself; `getPassword()` is the only way to read the actual current password (the stored real value if unedited, otherwise whatever's genuinely been typed) — `login()` now calls this instead of `.text()`. The first edit after a retrieved password was set wipes the field completely (discarding both the sentinel and that first keystroke) rather than allowing in-place editing around it, so the field can never end up holding a mix of the two. `_toggle_visibility` also gained a redundant enabled-check guard as belt-and-braces, though it's no longer load-bearing — the actual protection is that the real password was never in the widget to reveal in the first place. This closes the *entire* original finding, not just the toggle path: nothing that can read a `QLineEdit`'s text (clipboard, accessibility APIs, a future code bug elsewhere calling `.text()`) can recover the retrieved password anymore, since it's simply never there. Also removed `showHidePassword()`, an unrelated dead method (`self.btnShowPassword` was never created anywhere) discovered while in this code — not a live bypass, just misleading leftover code suggesting a second, unprotected toggle path that didn't actually exist.

**Still true, unchanged:** the field still silently receives the selected user's real password on load/selection (that's the "remember me" feature working as designed) — clicking Login without typing anything still authenticates as that remembered user. That's a product-level trade-off of the remember-me feature itself, not a bug this fix is scoped to address.
